...
#
# Usage:   ./scripts/harness-build.sh [env]
...
info "Building ${IMAGE_NAME}:latest (linux/amd64) ..."
docker buildx build \
  --platform linux/amd64 \
  --tag "${ECR_URI}:latest" \
  --tag "${ECR_URI}:${BUILD_DATE}" \
  --push \
  "${REPO_ROOT}/docker/testing-harness"
success "Pushed ${ECR_URI}:latest (and :${BUILD_DATE})"
